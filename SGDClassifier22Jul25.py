...
def categorySet():
    ...
    '''
    categories = json.load(open("../vars/categories_1Nov2022.json"))
...
def processData():
    ...
    y, valsToNums = pd.factorize(unprocessed["Categorization"])
    valsToNums = valsToNums.tolist()
...
def test(clf, valsToNums):
    ...
    mapping = {val: idx for idx, val in enumerate(valsToNums)}
    
    test["Categorization"] = test["Categorization"].map(mapping)
...
if __name__ == "__main__":
    ...
    if args.train == "TRUE":
        ...
        purposeCodes["Linted_Pur"] = purposeCodes[args.csv_column].str.replace(r"\d+", "")
